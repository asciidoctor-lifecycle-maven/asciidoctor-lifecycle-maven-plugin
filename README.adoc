...
It uses the https://maven.apache.org/wagon/wagon-provider-api/[wagon-provider-api], so all you have to do is configure the dependency (if needed).

I've tested it to upload files to a webdav server or a directory in your file system,
...
=== How to configure webdav for uploading
...
=== How to configure a path in your filesystem for uploading

       <uploadToRepository>file://${project.build.directory}/file-repository</uploadToRepository>
