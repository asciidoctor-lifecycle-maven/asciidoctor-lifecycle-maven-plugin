...
.2019/08/16
* In the theme-download phase for each downloaded theme,
...
All these operations are done at `theme-download` phase, so using the Asciidoctor Lifecycle
...
[.text-center]
`groupId:artifactId:zip:3.3.3`, for example.
    ...
    <executions>
        <execution>
            ...
            <configuration>
                ...
                <attributes>
                    <!-- 
                        The property "asciidoctor.theme.theme-example-1.path" is created at theme-download phase,
...
created at `theme-download` phase.

After the `theme-download` phase execution you'll get:
